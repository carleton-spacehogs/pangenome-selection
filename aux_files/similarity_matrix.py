...
for line in inFile.readlines()[1:]:
	...
	if cluster_id not in countCluster:
		countCluster[cluster_id] = [0]*22 #initializing
		countCluster[cluster_id][genome_number] += 1
	else:
		countCluster[cluster_id][genome_number] += 1

countGenome = [[0 for x in range(22)] for y in range(22)] #create a matrix of 0s
for cluster_id in countCluster: #the for loop that does the count
	for i in range(22):
...
#then transform the count into proportion
...
#create a function to make writing a txt file easier
